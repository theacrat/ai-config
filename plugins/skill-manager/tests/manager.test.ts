...
describe("native catalogue", () => {
  ...
  it("uses manifest aliases as IDs, keeps display names and original resource bases", () => {
    ...
    write(
      root,
      "sources.json",
      JSON.stringify({ skills: [{ name: "long-alias", root: "sources/upstream", path: "short" }] }),
    );
    const skill = loadCatalogue(root).find((entry) => entry.id === "long-alias");
    ...
    expect(readFileSync(join(dirname(skill?.path ?? ""), "references/rules.md"), "utf8")).toBe(
      "resource body",
    );
  });
  it("preserves external overrides and unrelated entries; retires old managed IDs", () => {
    const root = fixture();
    const project = parseSkill("Project override", "/other/.opencode/skills/task/SKILL.md", "task");
    const unrelated = {
      ...parseSkill("Unrelated", "/other/.opencode/skills/other/SKILL.md", "other"),
      autoinvoke: true,
    };
    const stale = parseSkill("Old managed copy", join(root, "old/SKILL.md"), "old");
  ...
  it("replays refreshed additions, edits and removals without changing prior snapshots", () => {
    ...
    expect(before.find((entry) => entry.id === "first")?.content).toBe("Before");
    expect(loadCatalogue(root).find((entry) => entry.id === "first")?.content).toBe("After");
    rmSync(dirname(path), { recursive: true });
    expect(loadCatalogue(root).map((entry) => entry.id)).toEqual(["second", "skill-discovery"]);
  });
...
describe("metadata search", () => {
  ...
  it("paginates deterministically and never returns bodies or paths", () => {
    expect(searchCatalogue(entries, { query: "task", offset: 10, limit: 2 })).toEqual([
    ...
    expect(() => searchCatalogue(entries, { query: "task", limit: 11 })).toThrow();
  ...
  it("finds exact IDs and keywords with bounded description lengths", () => {
    expect(searchCatalogue(entries, { query: "task-101" }).map((entry) => entry.id)).toEqual([
      "task-101",
    ]);
    expect(searchCatalogue(entries, { query: "TypeScript review", limit: 1 })).toHaveLength(1);
    expect(
      searchCatalogue([{ id: "long", name: "x".repeat(900), description: "y".repeat(900) }], {
        query: "long",
      })[0]?.description,
    ).toHaveLength(500);
  });
