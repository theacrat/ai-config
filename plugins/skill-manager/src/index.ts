  ...
  async setup(ctx) {
    ...
    const tool = await ctx.tool.transform((editor) => {
      editor.add({
        ...
        execute: async (input) => {
          ...
          const managed = new Map(catalogue.map((entry) => [String(entry.id), String(entry.path)]));
          return {
            content: JSON.stringify(
              searchCatalogue(
                current.data.filter((entry) => managed.get(entry.id) === entry.path),
                input,
              ),
            ),
          };
    ...
    const timer = setInterval(() => {
      try {
        ...
        void ctx.skill
          .reload()
          .catch((error: unknown) => console.error("skill-manager refresh failed", error));
      } catch (error) {
