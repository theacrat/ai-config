...
function editorFixture(initial: ProviderRecord[] = []) {
	const records = new Map(
		initial.map((record) => [record.provider.id, record]),
	);
...
describe("discovery", () => {
	it("carries endpoint pricing tiers, release dates and status into V2 provider models", async () => {
		...
		for (const model of models?.values() ?? [])
			expect(() => Model.Info.make(model)).not.toThrow();
	});
	...
	it("registers V2 source definitions, retains manual models/settings and disposes its registration", async () => {
		...
		const editor = editorFixture([
			{ provider: info, models: new Map([[manual.id, manual]]) },
		]);
	...
	it("uses the environment fallback and gives explicit options precedence", async () => {
		...
		expect(
			(await discover({}, () => {})).map((item) => item.source.id),
		).toEqual(["env"]);
	...
	it("keeps failed V2 sources intact while adding authenticated healthy sources", async () => {
		...
		expect([...(editor.get("healthy")?.models.keys() ?? [])]).toEqual([
			"new-model",
		]);
	...
	it("times out while reading a response body and accepts an empty catalogue", async () => {
		...
		expect(result.map((item) => [item.source.id, item.models.size])).toEqual([
			["empty", 0],
		]);
	...
	it("isolates HTTP, malformed JSON, invalid metadata, missing auth, redirect and timeout failures", async () => {
		...
		const baseURL = await endpoint((request, response) => {
			...
			switch (request.url) {
				...
				case "/metadata":
					response.end(
						'{"data":[{"id":"valid"},{"id":"bad","context_length":0}]}',
					);
					break;
				case "/redirect":
		...
		const result = await discover(
			{
				sources: [
					...["http", "json", "metadata", "redirect", "slow", "ok"].map(
						(id) => ({
							id,
							baseURL,
	...
	it.each(["__proto__", "prototype", "constructor", "coder#q4", " coder"])(
		...
		async (id) => {
			...
			const diagnostics: Diagnostic[] = [];
			expect(
				await discover({ sources: [{ id: "local", baseURL }] }, (value) =>
					diagnostics.push(value),
				),
			).toEqual([]);
			expect(diagnostics).toEqual([
				{ code: "invalid-response", sourceIndex: 0 },
			]);
		},
	);
	...
	it("rejects duplicate source/model IDs and invalid options with sanitised diagnostics", async () => {
		...
		for (const sources of [
			...
			...[
				...
				"coder#q4",
				" coder",
			].map((id) => [{ id: "local", baseURL, models: [id] }]),
			...[
				{ id: "a", context: 0 },
				{ id: "a", output: "100" },
				{ id: "a", tools: null },
			...
			const diagnostics: Diagnostic[] = [];
			expect(
				await discover({ sources }, (value) => diagnostics.push(value)),
			).toEqual([]);
			expect(diagnostics).toEqual([{ code: "invalid-options" }]);
		}
		expect(
			await discover({ sources: [{ id: "local", baseURL }] }, () => {}),
		).toEqual([]);
		vi.stubEnv("OPENCODE_MODEL_DISCOVERY", "secret bad JSON");
		const diagnostics: Diagnostic[] = [];
		expect(
			await discover(undefined, (value) => diagnostics.push(value)),
		).toEqual([]);
		expect(diagnostics).toEqual([{ code: "invalid-options" }]);
	});
	...
	it("keeps slash-containing model IDs selectable through OpenCode's public reference parser", async () => {
		...
		const refs = inventories.flatMap((inventory) =>
			[...inventory.models.keys()].map((id) =>
				Model.Ref.parse(`${inventory.source.id}/${id}`),
			),
		);
	...
	it.each([404, 405, 501, 401, 403, 500])(
		...
		async (status) => {
			...
			expect(
				inventories.map((item) => [item.source.id, [...item.models.values()]]),
			).toEqual([
				[
					"configured",
					[
	...
	it.each([
		"bad JSON",
		'{"data":[{"id":"bad","supports_tools":null}]}',
		'{"data":[]}',
	])("retains configured models for catalogue %s", async (body) => {
	...
	it("skips HTTP with discovery disabled, keeps empty sources and still requires credentials", async () => {
		...
		expect(
			inventories.map((item) => [item.source.id, [...item.models.keys()]]),
		).toEqual([
